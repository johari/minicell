...
testWidget :: Widget ()
testWidget = withBorderStyle unicode $
  borderWithLabel (str "Hello!") $
    (center (str "Left") <+> vBorder <+> center (str "Right"))

main :: IO ()
main = do
  h <- hamiltonian 4 [(0,1), (1,2), (2,3), (3,0), (1,3)]
  run
  simpleMain (ui $ show h)
  simpleMain (testWidget)
...
hamiltonian n edges = extractModel `fmap` satWith z3 hcycle
  where isEdge p = bAny (.== p) [(literal x, literal y) | (x, y) <- edges]
        validNodes xs = distinct xs &&& bAll (\x -> x .>= 0 &&& x .< fromIntegral n) xs
