...
type alias Demonstrations =
    { vertexDemos : List VertexAndPerhapsCells, edgeDemos : TEdgeDemo }


type alias Situation =
    { database : Database, demos : Demonstrations }


emptyDemonstration =
    { vertexDemos = [], edgeDemos = [] }
...
type alias VertexAndPerhapsCells =
    ( EExpr, List Cell )
...
type alias G =
    Graph VertexAndPerhapsCells EdgeLabel


type alias SuperEdge =
    ( VertexAndPerhapsCells, VertexAndPerhapsCells, EdgeLabel )


type alias TVertexDemo =
...
type EExpr
    = EApp EFunctor (List EExpr) -- CellFormula, I guess..
    | EILit Int -- CellInt
...
emptyCell : Cell
emptyCell =
    { value = EBot, buffer = "", addr = ( 0, 0 ), meta = Nothing, formula = "" }


stringCell addr str =
    { emptyCell | addr = addr, value = ESLit str }


intCell addr i =
    { emptyCell | addr = addr, value = EILit i }


imageCell addr src =
    { emptyCell | addr = addr, value = EImage src }


graphCell addr g =
    { emptyCell | addr = addr, value = EGraph emptyGraph }


formulaCell addr formula args =
    { emptyCell | addr = addr, value = EApp formula args }


cometCell addr endpoint =
    { emptyCell | addr = addr, value = EComet endpoint }
...
emptySpreadsheet =
    Spreadsheet [] (IdleMode ( 0, 0 )) [] [] (millisToPosix 0) [] (Dict.fromList []) "/"
